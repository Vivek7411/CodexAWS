  ...
  useEffect(() => {
      ...
      try {
        ...
        socketRef.current.on(ACTIONS.CURSOR_UPDATE, ({ username, position }) => {
          ...
          if (existingClient) {
            ...
            cursorElement.style.left = position.left + "px";
            cursorElement.style.position = "absolute";
            cursorElement.innerText = username.charAt(0);
            document.querySelector(".editorWrap").appendChild(cursorElement);
          }
        });
      } catch (error) {
    ...
    return () => {
      if (socketRef.current) {
        ...
        socketRef.current.off(ACTIONS.JOINED);
        socketRef.current.off(ACTIONS.DISCONNECTED);
        socketRef.current.off(ACTIONS.CURSOR_UPDATE); // Cleanup cursor event
      }
    };
  }, [location.state?.username, reactNavigator, roomId]);
  ...
  return (
    <div className="mainWrap">
      <div className="aside">
        ...
        <button className="btn copyBtn" onClick={() => navigator.clipboard.writeText(roomId)}>
          Copy ROOM ID
        </button>
        <button className="btn leaveBtn" onClick={() => reactNavigator("/")}>
          Leave
        </button>
        <button className="btn runBtn" onClick={runCode}>
          Run Code
